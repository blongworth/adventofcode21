...
f06a <- function(x, days) {
  fishes <- parser(x)
  # idea to not store individual fish as an array but
  # use vector of ages instead from David Robinson
  pop <- rep(0L, 9)
  for(fish in fishes) {
    pop[fish + 1] <- pop[fish + 1] + 1
  }

  # popping first element from stack from Joseph Chou's
  # beautiful Julia solution
  for (day in 1:days) {
    num0 <- pop[1]
    pop <- pop[-1]
    pop <- c(pop, num0)
