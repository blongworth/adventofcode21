...
#' and `0`, respectively, and so the final three bits of the gamma rate arekjlkj
#' `110`.
#'
...
#' Then: Keep only numbers selected by the bit criteria for the type of
#' rating value for which you are searching. Discard numbers which do not
#' match the bit criteria. If you only have one number left, stop; this is
#' the rating value for which you are searching. Otherwise, repeat the
#' process, considering the next bit to the right. The bit criteria depends
#' on which type of rating value you want to find: To find oxygen generator
#' rating, determine the most common value (0 or 1) in the current bit
#' position, and keep only numbers with that bit in that position. If 0 and
#' 1 are equally common, keep values with a 1 in the position being
#' considered. To find CO2 scrubber rating, determine the least common
#' value (0 or 1) in the current bit position, and keep only numbers with
#' that bit in that position. If 0 and 1 are equally common, keep values
#' with a 0 in the position being considered. For example, to determine the
#' oxygen generator rating value using the same example diagnostic report
#' from above: Start with all 12 numbers and consider only the first bit of
#' each number. There are more 1 bits (7) than 0 bits (5), so keep only the
#' 7 numbers with a 1 in the first position: 11110, 10110, 10111, 10101,
#' 11100, 10000, and 11001. Then, consider the second bit of the 7
#' remaining numbers: there are more 0 bits (4) than 1 bits (3), so keep
#' only the 4 numbers with a 0 in the second position: 10110, 10111, 10101,
#' and 10000. In the third position, three of the four numbers have a 1, so
#' keep those three: 10110, 10111, and 10101. In the fourth position, two
#' of the three numbers have a 1, so keep those two: 10110 and 10111. In
#' the fifth position, there are an equal number of 0 bits and 1 bits (one
#' each). So, to find the oxygen generator rating, keep the number with a 1
#' in that position: 10111. As there is only one number left, stop; the
#' oxygen generator rating is 10111, or 23 in decimal. Then, to determine
#' the CO2 scrubber rating value from the same example above: Start again
#' with all 12 numbers and consider only the first bit of each number.
...
#' there is only one number left, stop; the CO2 scrubber rating is 01010,
#' or 10 in decimal. Finally, to find the life support rating, multiply the
...
f03a <- function(x) {
  ...
  #separate
  sep <- x %>%
    separate(x, paste0('d', 1:dig), sep = 1:dig)
  gamma <- sep %>%
    summarise(across(everything(), ~ifelse(mean(as.numeric(.))>.5,1,0))) %>%
    unite(ans, everything(), sep='') %>%
    pull(ans) %>%
    strtoi(2)
  power <- sep %>%
    summarise(across(everything(), ~ifelse(mean(as.numeric(.))<.5,1,0))) %>%
    ...
    pull(ans) %>%
    strtoi(2)
  c("gamma" = gamma, "power" = power, "ans" = gamma * power)
}
...
f03b_ox <- function(x) {
  ...
  x %>%
    ...
    pull(ans) %>%
  strtoi(2)
  #maybe for char in length(x[[1,1]])
  # something recursive? Recursive filter?
}
...
f03b_ans <- function(x) {
  ox = f03b_ox(x)
  co2 = f03b_co2(x)
  c("ox" = ox, "co2" = co2, "ans" = ox * co2)

}

# stolen from Tan Ho's solution. I could not get my head around this.
...
f03_helper <- function(x) {
  read_table("inst/input03.txt", col_names = "x")
}
