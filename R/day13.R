...
#' f13b()
f13a <- function(x) {
  input <- paper_parser(x)
  fold(input$points, input$folds[[1,1]], input$folds[[1,2]]) %>%
    nrow()
}
...
f13b <- function(x) {
  input <- paper_parser(x)
  result <- input$points
  for (i in 1:nrow(input$folds)) {
    result <- fold(result, input$folds$axis[[i]], input$folds$value[[i]])
  }
...
paper_parser <- function(x) {
  points <- x[1:which(x == "")-1] %>%
    strsplit(",") %>% {
      tibble(x = map_chr(., 1),
             y = map_chr(., 2))
      } %>%
  ...
  folds <- x[(which(x == "")+1):length(x)] %>%
    tibble(x = .) %>%
    extract(x, c("axis", "value"), "(x|y)=(\\d+)") %>%
    mutate(value = as.integer(value))

  list(points = points, folds = folds)
}

# Ildiko Czeller's solution was a huge help here.
fold <- function(points, axis, value) {
  points %>%
    mutate(x = ifelse(axis == "y" | x <= value, x, 2L * value - x),
           y = ifelse(axis == "x" | y <= value, y, 2L * value - y)) %>%
    distinct()
}
